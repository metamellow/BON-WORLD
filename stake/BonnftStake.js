  ...
  abi: [
    {
      inputs: [
        ...
        { internalType: 'address', name: '_nftsAddress', type: 'address' },
        { internalType: 'address', name: '_devWallet', type: 'address' },
        { internalType: 'address', name: '_burnWallet', type: 'address' },
        { internalType: 'uint256', name: '_devRate', type: 'uint256' },
        { internalType: 'uint256', name: '_burnRate', type: 'uint256' },
        { internalType: 'uint256', name: '_claimPace', type: 'uint256' },
        { internalType: 'uint256', name: '_claimRate', type: 'uint256' },
        { internalType: 'bool', name: '_taxOn', type: 'bool' },
      ],
    ...
    },
    {
      inputs: [
        ...
        { internalType: 'uint256', name: '_devRate', type: 'uint256' },
        { internalType: 'uint256', name: '_burnRate', type: 'uint256' },
        { internalType: 'bool', name: '_trueFalse', type: 'bool' },
      ],
      name: 'updateTax',
      outputs: [],
      stateMutability: 'nonpayable',
...
class ClaimerDappInterface {
  ...
  async dappInitializeProcess() {
    // --- PRIMARY components
    try {
      await this.buttonManagerFunc();
    } catch (error) {
      console.log(error);
    }
    try {
      await this.pullUsersWallet();
    } catch (error) {
      console.log(error);
    }
    try {
      await this.connectAllContracts();
    } catch (error) {
      console.log(error);
    }
    try {
    ...
    try {
      await this.findUserNFTs();
    } catch (error) {
      console.log(error);
    }
    try {
      await this.estimateRewards();
    } catch (error) {
      console.log(error);
    }
    ...
    try {
      ...
      if (ethereum) {
        ...
        if (chainId !== this.dappChain) {
          ...
          alert(
            `Please use ${this.chainName} and REFRESH browser -- Other networks will NOT WORK!`
          );
        } else {
  ...
  async findUserNFTs() {
    ...
    try {
      ...
      let Function1Results = await this.connectedContract2.balanceOf(
        `${this.currentAccount}`
      );
      ...
      if (Function1Results != '') {
        ...
        this.buttonsArray[1].innerText = `no results`;
      }
    } catch (error) {
  ...
  async claimRewards() {
    ...
    try {
      ...
      let provider = new ethers.providers.JsonRpcProvider(
        `https://polygon-mainnet.g.alchemy.com/v2/yZJgARTBAasEklX3RTd-bXHN40bIOgwi`
      );
      ...
      let lastClaimTime = await this.connectedContract1.lastClaimTime(
        `${this.selectedInput1}`
      );
    ...
    try {
      ...
      let Button2Results = await this.connectedContract1.claim(
        `${this.selectedInput1}`
      );
    } catch (error) {
  ...
  displayNewNFTData(data) {
    ...
    let mapped = resultsArray.map(item => {
      return this.createNFTElement(item);
    });
